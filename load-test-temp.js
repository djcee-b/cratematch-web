...
const CONFIG = {
  ...
  concurrentUsers: parseInt(process.env.CONCURRENT_USERS) || 5, // Temporarily reduced to test auth fix
  rampUpTime: parseInt(process.env.RAMP_UP_TIME) || 10, // seconds
  testUser: {
    email: process.env.TEST_EMAIL || "djceebweb@gmail.com",
    password: process.env.TEST_PASSWORD || "Terry1234",
  },
...
// Create authenticated session with unique user credentials
async function createAuthenticatedSession(userId) {
  ...
  // Create unique credentials for each user to avoid authentication conflicts
  const uniqueUser = {
    email: `loadtest${userId}@example.com`,
    password: `loadtest${userId}123`,
  };

  try {
    // Try to sign in first
    const signInResponse = await session.post("/auth/signin", uniqueUser);
    ...
    const signUpResponse = await session.post("/auth/signup", uniqueUser);

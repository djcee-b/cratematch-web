...
async function testAuth() {
  ...
  try {
    // Test signup
    const signupResponse = await axios.post(`${CONFIG.baseURL}/auth/signup`, {
      email: user.email,
    ...
    // Test signin
    const signinResponse = await axios.post(`${CONFIG.baseURL}/auth/signin`, {
      email: user.email,
      password: user.password
    }, {
      timeout: 10000,
...
async function testProcessing() {
  ...
  try {
    ...
    const response = await axios.post(`${CONFIG.baseURL}/api/process-playlist`, {
...
async function runLoadTest() {
  ...
  const userSessions = Array.from({ length: CONFIG.concurrentUsers }, (_, i) => i);
  ...
  const sessionPromises = userSessions.map(async (userId) => {
    while (Date.now() < endTime) {
      ...
      if (rand < CONFIG.testScenarios.homepage) {
        scenario = 'homepage';
      } else if (rand < CONFIG.testScenarios.homepage + CONFIG.testScenarios.auth) {
        scenario = 'auth';
      } else if (rand < CONFIG.testScenarios.homepage + CONFIG.testScenarios.auth + CONFIG.testScenarios.app) {
        scenario = 'app';
      } else {
        scenario = 'processing';
      }
...
function generateReport() {
  ...
  Object.entries(metrics.scenarios).forEach(([scenario, data]) => {
    const scenarioSuccessRate = data.requests > 0 
      ...
      : 0;
    console.log(`   ${scenario}: ${data.requests} requests, ${data.errors} errors (${scenarioSuccessRate}% success)`);
  });
  console.log('');
  ...
  if (p95 > 5000) {
    console.log('   ⚠️  P95 response time above 5s - consider performance optimizations');
  }
  if (avgResponseTime > 2000) {
    console.log('   ⚠️  Average response time above 2s - investigate slow endpoints');
  }
